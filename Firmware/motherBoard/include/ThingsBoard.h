...
constexpr char RPC_RESPONSE_SUBSCRIBE_TOPIC[] PROGMEM =
    "v1/devices/me/rpc/response/+";
constexpr char RPC_SEND_REQUEST_TOPIC[] PROGMEM =
    "v1/devices/me/rpc/request/%u";
...
#if THINGSBOARD_ENABLE_PROGMEM
constexpr char ATTRIBUTE_REQUEST_TOPIC[] PROGMEM =
    "v1/devices/me/attributes/request/%u";
constexpr char ATTRIBUTE_RESPONSE_SUBSCRIBE_TOPIC[] PROGMEM =
    "v1/devices/me/attributes/response/+";
constexpr char ATTRIBUTE_RESPONSE_TOPIC[] PROGMEM =
    "v1/devices/me/attributes/response";
#else
constexpr char ATTRIBUTE_REQUEST_TOPIC[] =
    "v1/devices/me/attributes/request/%u";
constexpr char ATTRIBUTE_RESPONSE_SUBSCRIBE_TOPIC[] =
    "v1/devices/me/attributes/response/+";
...
constexpr char UNABLE_TO_DE_SERIALIZE_JSON[] PROGMEM =
    "Unable to de-serialize received json data with error (%s)";
constexpr char INVALID_BUFFER_SIZE[] PROGMEM =
    "Buffer size (%u) to small for the given payloads size (%u), increase with "
    "setBufferSize accordingly";
constexpr char NUMBER_PRINTF[] PROGMEM = "%u";
#if !THINGSBOARD_ENABLE_DYNAMIC
constexpr char MAX_RPC_EXCEEDED[] PROGMEM =
    "Too many server-side RPC subscriptions, increase MaxFieldsAmt or "
    "unsubscribe";
constexpr char MAX_RPC_REQUEST_EXCEEDED[] PROGMEM =
    "Too many client-side RPC subscriptions, increase MaxFieldsAmt or "
    "unsubscribe";
constexpr char MAX_SHARED_ATT_UPDATE_EXCEEDED[] PROGMEM =
    "Too many shared attribute update callback subscriptions, increase "
    "MaxFieldsAmt or unsubscribe";
constexpr char MAX_SHARED_ATT_REQUEST_EXCEEDED[] PROGMEM =
    "Too many shared attribute request callback subscriptions, increase "
    "MaxFieldsAmt";
...
constexpr char REQUEST_RPC[] PROGMEM =
    "Requesting client side RPC with the json (%s)";
constexpr char REQUEST_ATT[] PROGMEM =
    "Requesting shared attributes transformed from (%s) into json (%s)";
...
constexpr char NO_RPC_PARAMS_PASSED[] PROGMEM =
    "No parameters passed with RPC, passing null JSON";
constexpr char CALLING_RPC[] PROGMEM = "Calling RPC:";
constexpr char RECEIVED_ATT_UPDATE[] PROGMEM =
    "Received shared attribute update";
constexpr char NOT_FOUND_ATT_UPDATE[] PROGMEM =
    "Shared attribute update key not found";
constexpr char ATT_CB_NO_KEYS[] PROGMEM =
    "No keys subscribed. Calling subscribed callback for any updated "
    "attributes (assumed to be subscribed to every possible key)";
constexpr char ATT_IS_NULL[] PROGMEM =
    "Subscribed shared attribute update key is NULL";
constexpr char ATT_IN_ARRAY[] PROGMEM =
    "Shared attribute update key: (%s) is subscribed";
...
constexpr char CALLING_REQUEST_CB[] PROGMEM =
    "Calling subscribed callback for response id (%u)";
constexpr char CB_ON_MESSAGE[] PROGMEM =
    "Callback onMQTTMessage from topic: (%s)";
constexpr char SUBSCRIBE_TOPIC_FAILED[] PROGMEM =
    "Subscribing the given topic failed";
...
constexpr char ATT_CB_NO_KEYS[] =
    "No keys subscribed. Calling subscribed callback for any updated "
    "attributes (assumed to be subscribed to every possible key)";
constexpr char ATT_IS_NULL[] = "Subscribed shared attribute update key is NULL";
constexpr char ATT_IN_ARRAY[] =
    "Shared attribute update key: (%s) is subscribed";
constexpr char ATT_NO_CHANGE[] =
    "No keys that we subscribed too were changed, skipping callback";
constexpr char CALLING_ATT_CB[] =
    "Calling subscribed callback for updated shared attribute (%s)";
...
constexpr char FW_NOT_FOR_US[] PROGMEM =
    "Firmware is not for us (title is different)";
constexpr char UNABLE_TO_WRITE[] PROGMEM = "Unable to write firmware";
constexpr char UNABLE_TO_REQUEST_CHUNCKS[] PROGMEM =
    "Unable to request firmware chunk";
...
constexpr char ERROR_UPDATE_END[] PROGMEM =
    "Error during Update.end, not all bytes written";
constexpr char CHKS_VER_FAILED[] PROGMEM = "Checksum verification failed";
constexpr char FW_CHKS_ALGO_NOT_SUPPORTED[] PROGMEM =
...
constexpr char ERROR_UPDATE_END[] =
    "Error during Update.end, not all bytes written";
constexpr char CHKS_VER_FAILED[] = "Checksum verification failed";
constexpr char FW_CHKS_ALGO_NOT_SUPPORTED[] =
    "Checksum algorithm (%s) is not supported";
...
#if THINGSBOARD_ENABLE_DYNAMIC
/// @brief Wrapper around the PubSubClient to allow connecting and sending /
/// retrieving data from ThingsBoard over the MQTT or MQTT with TLS/SSL
/// protocol. BufferSize of the underlying data buffer as well as the maximum
/// amount of data points that can ever be sent are either dynamic or can be
/// changed during runtime. If this feature is not needed and the values can be
/// sent once as template arguements it is recommended to use the static
/// ThingsBoard instance instead. Simply set THINGSBOARD_ENABLE_DYNAMIC to 0,
/// before including ThingsBoard.h
/// @tparam Logger Logging class that should be used to print messages generated
/// by ThingsBoard, default = ThingsBoardDefaultLogger
template <typename Logger = ThingsBoardDefaultLogger>
#else
/// @brief Wrapper around the PubSubClient to allow connecting and sending /
/// retrieving data from ThingsBoard over the MQTT or MQTT with TLS/SSL
/// protocol. BufferSize of the underlying data buffer as well as the maximum
/// amount of data points that can ever be sent have to defined as template
/// arguments. Changing is only possible if a new instance of this class is
/// created. If theese values should be changeable and dynamic instead. Simply
/// set THINGSBOARD_ENABLE_DYNAMIC to 1, before including ThingsBoard.h
/// @tparam MaxFieldsAmt Maximum amount of key value pair that we will be able
/// to sent to ThingsBoard in one call, default = 8
/// @tparam Logger Logging class that should be used to print messages generated
/// by ThingsBoard, default = ThingsBoardDefaultLogger
template <size_t MaxFieldsAmt = 64, typename Logger = ThingsBoardDefaultLogger>
#endif  // THINGSBOARD_ENABLE_DYNAMIC
class ThingsBoardSized {
 public:
  /// @brief Constructs a ThingsBoardSized instance with the given network
  /// client
  /// @param client Network client that should be used to establish the
  /// connection to ThingsBoard
  /// @param enableQoS Whether the PubSubClient should use Quality of Service
  /// Level 1 or not, false means Level 0 is used, which means we simply send
  /// the data and hope it arrives if it is lost in transit it isn't resent.
  /// Level 2 however stores the given message we wanted to send until a
  /// response is received from the server, meaning that the message has been
  /// sent at least once, but it could have been sent multiple times as well,
  /// will not be a problem if the optional timestamp is added to the sent data,
  /// making duplicate sent data irrelevant because it simply overrides the data
  /// with the same timestamp, see
  /// https://www.hivemq.com/blog/mqtt-essentials-part-6-mqtt-quality-of-service-levels/
  /// for more information
  /// @param bufferSize Maximum amount of data that can be either received or
  /// sent to ThingsBoard at once, if bigger packets are received they are
  /// discarded and if we attempt to send data that is bigger, it will not be
  /// sent, can be changed later with the setBufferSize() method
  /// @param maxStackSize Maximum amount of bytes we want to allocate on the
  /// stack, default = Default_Max_Stack_Size
  inline ThingsBoardSized(Client &client,
                          const uint16_t &bufferSize = Default_Payload,
                          const bool &enableQoS = false,
                          const uint32_t &maxStackSize = Default_Max_Stack_Size)
  ...
  /// @brief Constructs a ThingsBoardSized instance without a network client,
  /// meaning it has to be added later with the setClient() and setBufferSize()
  /// method
  /// @param enableQoS Whether the PubSubClient should use Quality of Service
  /// Level 1 or not, false means Level 0 is used, which means we simply send
  /// the data and hope it arrives if it is lost in transit it isn't resent.
  ...
  inline ThingsBoardSized(const bool &enableQoS = false)
      : m_client(),
        ...
        m_fwAlgorithm(),
        m_fwChecksum(),
        m_fwSharedKeys{FW_CHKS_KEY, FW_CHKS_ALGO_KEY, FW_SIZE_KEY, FW_TITLE_KEY,
                       FW_VER_KEY},
        m_fwRequestCallback(
            m_fwSharedKeys.cbegin(), m_fwSharedKeys.cend(),
            std::bind(&ThingsBoardSized::Firmware_Shared_Attribute_Received,
  ...
  /// allows for calling method directly on the client itself, not advised in
  /// normal use cases, as it might cause problems if the library expects the
  /// client to be sending / receiving data but can't anymore because it has
  /// been disconnected or certain settings were changed
  /// @return Reference to the underlying PubSubClient connected to ThingsBoard
  inline PubSubClient &getClient() { return m_client; }

  /// @brief Sets the Quality of Service Level used by the MQTT client either
  /// true for Level 1 or false for Level 0
  /// @param enableQoS  Whether the PubSubClient should use Quality of Service
  /// Level 1 or not, false means Level 0 is used, which means we simply send
  /// the data and hope it arrives if it is lost in transit it isn't resent.
  /// Level 2 however stores the given message we wanted to send until a
  ...
  /// https://www.hivemq.com/blog/mqtt-essentials-part-6-mqtt-quality-of-service-levels/
  /// for more information
  inline void enableMQTTQoS(const bool &enableQoS) { m_qos = enableQoS; }

  /// @brief Sets the underlying network client that should be used to establish
  /// the connection to ThingsBoard, ensure to disconnect and connect again
  /// after changing the underlying client
  /// @param client Network client that should be used to establish the
  /// connection to ThingsBoard
  inline void setClient(Client &client) {
    ...
    // Initalize callback.
#if THINGSBOARD_ENABLE_STL
    m_client.setCallback(std::bind(&ThingsBoardSized::onMQTTMessage, this,
  ...
  /// @brief Sets the maximum amount of bytes that we want to allocate on the
  /// stack, before allocating on the heap instead
  /// @param maxStackSize Maximum amount of bytes we want to allocate on the
  /// stack
  ...
  /// @brief Sets the size of the buffer for the underlying network client that
  /// will be used to establish the connection to ThingsBoard
  /// @param bufferSize Maximum amount of data that can be either received or
  /// sent to ThingsBoard at once, if bigger packets are received they are
  /// discarded and if we attempt to send data that is bigger, it will not be
  /// sent
  /// @return Whether allocating the needed memory for the given bufferSize was
  /// successful or not.
  ...
  const char *TB_host;
  const char *TB_token;
  /// @brief Connects to the specified ThingsBoard server over the given port as
  /// the given device
  /// @param host ThingsBoard server instance we want to connect to
  /// @param access_token Access token that connects this device with a created
  /// device on the ThingsBoard server, can be "provision", if the device
  /// creates itself instead
  /// @param port Port that will be used to establish a connection and send /
  /// receive data from ThingsBoard over
  /// @param client_id Client username that can be used to differentiate the
  /// user that is connecting the given device to ThingsBoard
  /// @param password Client password that can be used to authenticate the user
  /// that is connecting the given device to ThingsBoard
  /// @return Whether connecting to ThingsBoard was successful or not
  inline bool connect(const char *host,
                      const char *access_token = PROV_ACCESS_TOKEN,
  ...
  inline bool connect(const IPAddress &host,
                      const char *access_token = PROV_ACCESS_TOKEN,
                      const uint16_t port = 1883,
                      const char *client_id = DEFAULT_CLIENT_ID,
                      const char *password = nullptr) {
  ...
  /// @brief Returns our current connection status to the cloud, true meaning we
  /// are connected, false meaning we have been disconnected or have not
  ...
  /// @brief Receives / sends any outstanding messages from and to the MQTT
  /// broker
  bool loop() { return (m_client.loop()); }

  //----------------------------------------------------------------------------
  // Claiming API

  /// @brief Sends a claiming request for the given device, allowing any given
  /// user on the cloud to assign the device as their own (claim), as long as
  /// they enter the given device name and secret key in the given amount of
  /// time. Optionally a secret key can be passed or be left empty (cloud will
  /// allow any user to claim the device for the given amount of time)
  /// @param secretKey Password the user additionaly to the device name needs to
  /// enter to claim it as their own, pass nullptr or an empty string if the
  /// user should be able to claim the device without any password
  /// @param durationMs Total time in milliseconds the user has to claim their
  /// device as their own
  ...
  inline bool Claim_Request(const char *secretKey, const uint32_t &durationMs) {
    ...
    if (currentBufferSize < objectSize) {
      char message[detectSize(INVALID_BUFFER_SIZE, currentBufferSize,
                              objectSize)];
      snprintf_P(message, sizeof(message), INVALID_BUFFER_SIZE,
                 currentBufferSize, objectSize);
      Logger::log(message);
      return false;
    }
    // Serialize json does not include size of the string null terminator
    else if (serializeJson(respObj, responsePayload, objectSize) <
             objectSize - 1) {
  ...
  /// @brief Sends provisioning request for a new device, meaning we want to
  /// create a device that we can then connect over, where the given provision
  /// device key / secret decide which device profile is used to create the
  /// given device with. Optionally a device name can be passed or be left empty
  /// (cloud will use the unqiue access token as a name instead). The cloud then
  /// sends back json data containing our credentials, that the given callback,
  /// if creating the device was successful. The data contained in that
  /// callbackcan then be used to disconnect and reconnect to the ThingsBoard
  /// server as our newly created device
  ...
  inline bool Provision_Request(const Provision_Callback &callback) {
    ...
    // meaning if it is an empty string or null instead we don't send it at all.
    // Deciding which underlying provisioning method is restricted, by the
    // Provision_Callback class. Meaning only the key-value pairs that are
    // needed for the given provisionign method are set, meaning the rest will
    // not be sent and therefore the provisoning request has the correct
    // formatting
    ...
    // Serialize json does not include size of the string null terminator
    else if (serializeJson(requestObject, requestPayload, objectSize) <
             objectSize - 1) {
      Logger::log(UNABLE_TO_SERIALIZE_JSON);
      return false;
    }
  ...
  /// @brief Attempts to send telemetry data with the given key and value of the
  /// given type
  ...
  /// @brief Attempts to send integer telemetry data with the given key and
  /// value
  /// @param key Key of the key value pair we want to send
  /// @param value Value of the key value pair we want to send
  /// @return Whether sending the data was successful or not
  ...
  /// @brief Attempts to send boolean telemetry data with the given key and
  /// value
  /// @param key Key of the key value pair we want to send
  /// @param value Value of the key value pair we want to send
  /// @return Whether sending the data was successful or not
  ...
  /// @param json String containing our json key value pairs we want to attempt
  /// to send
  /// @return Whether sending the data was successful or not
  inline bool sendTelemetryJson(const char *json) {
    ...
    if (currentBufferSize < jsonSize) {
      char
          message[detectSize(INVALID_BUFFER_SIZE, currentBufferSize, jsonSize)];
      snprintf_P(message, sizeof(message), INVALID_BUFFER_SIZE,
                 currentBufferSize, jsonSize);
      Logger::log(message);
      return false;
    }
  ...
  /// @param jsonObject JsonObject containing our json key value pairs we want
  /// to send
  /// @param jsonSize Size of the data inside the JsonObject
  /// @return Whether sending the data was successful or not
  inline bool sendTelemetryJson(const JsonObject jsonObject,
                                const uint32_t &jsonSize) {
    // Check if allocating needed memory failed when trying to create the
    // JsonObject, if it did the method will return true. See
    // https://arduinojson.org/v6/api/jsonobject/isnull/ for more information.
    ...
    if (MaxFieldsAmt < jsonObjectSize) {
      char message[detectSize(TOO_MANY_JSON_FIELDS, jsonObjectSize,
                              MaxFieldsAmt)];
      snprintf_P(message, sizeof(message), TOO_MANY_JSON_FIELDS, jsonObjectSize,
                 MaxFieldsAmt);
      Logger::log(message);
      return false;
    }
    ...
    // Check if the remaining stack size of the current task would overflow the
    // stack, if it would allocate the memory on the heap instead to ensure no
    ...
    if (getMaximumStackSize() < jsonSize) {
      ...
      // Ensure to actually delete the memory placed onto the heap, to make sure
      // we do not create a memory leak and set the pointer to null so we do not
  ...
  /// @param jsonVariant JsonVariant containing our json key value pairs we want
  /// to send
  /// @param jsonSize Size of the data inside the JsonVariant
  /// @return Whether sending the data was successful or not
  inline bool sendTelemetryJson(const JsonVariant &jsonVariant,
                                const uint32_t &jsonSize) {
    // Check if allocating needed memory failed when trying to create the
    // JsonObject, if it did the method will return true. See
    ...
    if (MaxFieldsAmt < jsonVariantSize) {
      char message[detectSize(TOO_MANY_JSON_FIELDS, jsonVariantSize,
                              MaxFieldsAmt)];
      snprintf_P(message, sizeof(message), TOO_MANY_JSON_FIELDS,
                 jsonVariantSize, MaxFieldsAmt);
      Logger::log(message);
      return false;
    }
#endif  // !THINGSBOARD_ENABLE_DYNAMIC
  ...
  /// @brief Attempts to send attribute data with the given key and value of the
  /// given type
  /// @tparam T Type of the passed value
  /// @param key Key of the key value pair we want to send
  /// @param value Value of the key value pair we want to send
  ...
  /// @brief Attempts to send integer attribute data with the given key and
  /// value
  /// @param key Key of the key value pair we want to send
  /// @param value Value of the key value pair we want to send
  /// @return Whether sending the data was successful or not
  ...
  /// @brief Attempts to send boolean attribute data with the given key and
  /// value
  /// @param key Key of the key value pair we want to send
  /// @param value Value of the key value pair we want to send
  /// @return Whether sending the data was successful or not
  ...
  /// @param jsonSize Size of the data inside the JsonVariant
  /// @return Whether sending the data was successful or not
  inline bool sendAttributeJSON(const JsonVariant &jsonVariant,
                                const uint32_t &jsonSize) {
    // Check if allocating needed memory failed when trying to create the
    // JsonObject, if it did the method will return true. See
    // https://arduinojson.org/v6/api/jsonvariant/isnull/ for more information.
  ...
  /// that will be called if the key-value pair from the server for the given
  /// client-side attributes is received
  /// @param callback Callback method that will be called
  /// @return Whether requesting the given callback was successful or not
  inline bool Client_Attributes_Request(
      const Attribute_Request_Callback &callback) {
    return Attributes_Request(callback, CLIENT_REQUEST_KEYS,
                              CLIENT_RESPONSE_KEY);
  }
  ...
  /// that will be called if a request from the server for the method with the
  /// given name is received
  /// @tparam InputIterator Class that points to the begin and end iterator
  /// of the given data container, allows for using / passing either std::vector
  /// or std::array
  /// @param first_itr Iterator pointing to the first element in the data
  /// container
  /// @param last_itr Iterator pointing to the end of the data container (last
  /// element + 1)
  /// @return Whether subscribing the given callbacks was successful or not
  template <class InputIterator>
  inline bool RPC_Subscribe(const InputIterator &first_itr,
  ...
  /// @param callbacksSize Amount of values that should be subscribed, ensure
  /// size matches the actual array, if not the system might crash unexpectedly
  /// at a later point
  /// @return Whether subscribing the given callbacks was successful or not
  inline bool RPC_Subscribe(const RPC_Callback *callbacks,
  ...
  /// that will be called if a response from the server for the method with the
  /// given name is received
  /// @param callback Callback method that will be called
  /// @return Whether requesting the given callback was successful or not
  inline bool RPC_Request(const RPC_Request_Callback &callback) {
...
#if THINGSBOARD_ENABLE_DYNAMIC
    // String are const char* and therefore stored as a pointer --> zero copy,
    // meaning the size for the strings is 0 bytes, Data structure size depends
    // on the amount of key value pairs passed + the default methodName and
    // params key needed for the request. See
    // https://arduinojson.org/v6/assistant/ for more information on the needed
    // size for the JsonDocument
    const uint32_t dataStructureMemoryUsage =
        JSON_OBJECT_SIZE(parameters != nullptr ? parameters->size() + 2U : 2U);
    TBJsonDocument requestBuffer(dataStructureMemoryUsage);
#else
    // Ensure to have enough size for the infinite amount of possible parameters
    // that could be sent to the cloud, therefore we set the size to the
    // MaxFieldsAmt instead of JSON_OBJECT_SIZE(1), which will result in a
    // JsonDocument with a size of 16 bytes
    ...
    char message[JSON_STRING_SIZE(strlen(REQUEST_RPC)) +
                 JSON_STRING_SIZE(strlen(buffer))];
  ...
  /// @brief Immediately starts a firmware update if firmware is assigned to the
  /// given device
  /// @param callback Callback method that will be called
  /// @return Whether subscribing the given callback was successful or not
  ...
  /// @brief Subscribes for any assignment of firmware to the given device
  /// device, which will then start a firmware update
  /// @param callback Callback method that will be called
  /// @return Whether subscribing the given callback was successful or not
  ...
  /// @return Whether sending the current device firmware information was
  /// successful or not
  inline bool Firmware_Send_Info(const char *currFwTitle,
                                 const char *currFwVersion) {
    StaticJsonDocument<JSON_OBJECT_SIZE(2)> currentFirmwareInfo;
    const JsonObject currentFirmwareInfoObject =
        currentFirmwareInfo.to<JsonObject>();
    ...
    return sendTelemetryJson(
        currentFirmwareInfoObject,
        JSON_STRING_SIZE(measureJson(currentFirmwareInfoObject)));
  }

  /// @brief Sends the given firmware state to the cloud
  /// @param currFwState Current firmware download state
  /// @param fwError Firmware error message that describes the current firmware
  /// state, pass nullptr or an empty string if the current state is not a
  /// failure state and therefore does not require any firmware error messsages
  /// @return Whether sending the current firmware download state was successful
  /// or not
  inline bool Firmware_Send_State(const char *currFwState,
                                  const char *fwError = nullptr) {
    StaticJsonDocument<JSON_OBJECT_SIZE(2)> currentFirmwareState;
    const JsonObject currentFirmwareStateObject =
        currentFirmwareState.to<JsonObject>();
    ...
    return sendTelemetryJson(
        currentFirmwareStateObject,
        JSON_STRING_SIZE(measureJson(currentFirmwareStateObject)));
  }
  ...
  /// that will be called if the key-value pair from the server for the given
  /// shared attributes is received
  /// @param callback Callback method that will be called
  /// @return Whether requesting the given callback was successful or not
  inline bool Shared_Attributes_Request(
      const Attribute_Request_Callback &callback) {
    return Attributes_Request(callback, SHARED_REQUEST_KEY,
                              SHARED_RESPONSE_KEY);
  }
  ...
  inline bool Shared_Attributes_Subscribe(const InputIterator &first_itr,
...
#if !THINGSBOARD_ENABLE_DYNAMIC
    const uint32_t size = std::distance(first_itr, last_itr);
    if (m_sharedAttributeUpdateCallbacks.size() + size >
        m_sharedAttributeUpdateCallbacks.capacity()) {
    ...
    // Push back complete vector into our local m_sharedAttributeUpdateCallbacks
    // vector.
    m_sharedAttributeUpdateCallbacks.insert(
        m_sharedAttributeUpdateCallbacks.end(), first_itr, last_itr);
    return true;
  }
  ...
  /// at a later point
  /// @return Whether subscribing the given callbacks was successful or not
  inline bool Shared_Attributes_Subscribe(
      const Shared_Attribute_Callback *callbacks, const size_t &callbacksSize) {
#if !THINGSBOARD_ENABLE_DYNAMIC
    if (m_sharedAttributeUpdateCallbacks.size() + callbacksSize >
        m_sharedAttributeUpdateCallbacks.capacity()) {
      Logger::log(MAX_SHARED_ATT_UPDATE_EXCEEDED);
      return false;
    }
  ...
  /// @return Whether subscribing the given callback was successful or not
  inline bool Shared_Attributes_Subscribe(
      const Shared_Attribute_Callback &callback) {
#if !THINGSBOARD_ENABLE_DYNAMIC
    if (m_sharedAttributeUpdateCallbacks.size() + 1U >
        m_sharedAttributeUpdateCallbacks.capacity()) {
      Logger::log(MAX_SHARED_ATT_UPDATE_EXCEEDED);
      return false;
    }
  ...
  /// @brief Returns the length in characters needed for a given value with the
  /// given argument string to be displayed completly
  /// @param msg Formating message that the given argument will be inserted into
  /// @param ... Additional arguments that should be inserted into the message
  /// at the given points, see https://cplusplus.com/reference/cstdio/printf/
  /// for more information on the possible arguments
  /// @return Length in characters, needed for the given message with the given
  /// values inserted to be displayed completly
  inline static uint8_t detectSize(const char *msg, ...) {
    va_list args;
    va_start(args, msg);
    // Result is what would have been written if the passed buffer would have
    // been large enough not counting null character, or if an error occured
    // while creating the string a negative number is returned instead. TO
    // ensure this will not crash the system when creating an array with
    // negative size we assert beforehand with a clear error message.
    const int32_t result =
        JSON_STRING_SIZE(vsnprintf_P(nullptr, 0U, msg, args));
#if THINGSBOARD_ENABLE_STL
    assert(result >= 0);
#else
    if (result < 0) {
  ...
  /// @brief Returns the amount of occurences of the given smybol in the given
  /// string
 ...
 private:
  /// @brief Returns the maximum amount of bytes that we want to allocate on the
  /// stack, before allocating on the heap instead
  /// @return Maximum amount of bytes we want to allocate on the stack
  inline const uint32_t &getMaximumStackSize() const { return m_maxStack; }

  /// @brief Requests one client-side or shared attribute calllback,
  /// that will be called if the key-value pair from the server for the given
  /// client-side or shared attributes is received
  /// @param callback Callback method that will be called
  /// @param attributeRequestKey Key of the key-value pair that will contain the
  /// attributes we want to request
  /// @param attributeResponseKey Key of the key-value pair that will contain
  /// the attributes we got as a response
  /// @return Whether requesting the given callback was successful or not
  inline bool Attributes_Request(const Attribute_Request_Callback &callback,
    ...
    if (attributes.empty()) {
      ...
      return false;
    } else if (attributeRequestKey == nullptr ||
               attributeResponseKey == nullptr) {
    ...
    // String are const char* and therefore stored as a pointer --> zero copy,
    // meaning the size for the strings is 0 bytes, Data structure size depends
    // on the amount of key value pairs passed + the default clientKeys or
    // sharedKeys See https://arduinojson.org/v6/assistant/ for more information
    // on the needed size for the JsonDocument
    ...
    // Serialize json does not include size of the string null terminator
    else if (serializeJson(requestVariant, buffer, objectSize) <
             objectSize - 1) {
      Logger::log(UNABLE_TO_SERIALIZE_JSON);
      return false;
    }
...
#if THINGSBOARD_ENABLE_STL
    char message[JSON_STRING_SIZE(strlen(REQUEST_ATT)) + request.length() +
                 JSON_STRING_SIZE(strlen(buffer))];
    snprintf_P(message, sizeof(message), REQUEST_ATT, request.c_str(), buffer);
#else
    char message[JSON_STRING_SIZE(strlen(REQUEST_ATT)) +
                 JSON_STRING_SIZE(strlen(request)) +
                 JSON_STRING_SIZE(strlen(buffer))];
  ...
  /// @param callback Callback method that will be called
  /// @return Whether checking and sending the current device firmware
  /// information was successful or not
  ...
  /// @return Whether subscribing to the firmware response topic was successful
  /// or not
  ...
  /// @brief Unsubscribes from the firmware response topic and clears any memory
  /// associated with the firmware update, should not be called before actually
  /// fully completing the firmware update.
  /// @return Whether unsubscribing from the firmware response topic was
  /// successful or not
  ...
  /// @param data Json data containing key-value pairs for the needed firmware
  /// information, to ensure we have a firmware assigned and can start the
  /// update over MQTT
  inline void Firmware_Shared_Attribute_Received(
      const Shared_Attribute_Data &data) {
    ...
    if (fw_title == nullptr || fw_version == nullptr ||
        currFwTitle == nullptr || currFwVersion == nullptr ||
        m_fwAlgorithm.empty() || m_fwChecksum.empty()) {
      Logger::log(EMPTY_FW);
      Firmware_Send_State(FW_STATE_FAILED, EMPTY_FW);
      return;
    }
    // If firmware version and title is the same, we do not initiate an update,
    // because we expect the binary to be the same one we are currently using
    else if (strncmp_P(currFwTitle, fw_title,
                       JSON_STRING_SIZE(strlen(currFwTitle))) == 0 &&
             strncmp_P(currFwVersion, fw_version,
                       JSON_STRING_SIZE(strlen(currFwVersion))) == 0) {
      Logger::log(FW_UP_TO_DATE);
      Firmware_Send_State(FW_STATE_FAILED, FW_UP_TO_DATE);
      return;
    }
    // If firmware title is not the same, we do not initiate an update, because
    // we expect the binary to be for another device type
    else if (strncmp_P(currFwTitle, fw_title,
                       JSON_STRING_SIZE(strlen(currFwTitle))) != 0) {
    ...
    // Change the used firmware algorithm, depending on which type is set for
    // the given firmware information
    if (m_fwAlgorithm.compare(CHECKSUM_AGORITM_MD5) == 0) {
      ...
      char message[JSON_STRING_SIZE(strlen(FW_CHKS_ALGO_NOT_SUPPORTED)) +
                   JSON_STRING_SIZE(m_fwAlgorithm.size())];
      snprintf_P(message, sizeof(message), FW_CHKS_ALGO_NOT_SUPPORTED,
                 m_fwAlgorithm.c_str());
      Logger::log(message);
    ...
    Logger::log(NEW_FW);
    char firmware[JSON_STRING_SIZE(strlen(FROM_TOO)) +
                  JSON_STRING_SIZE(strlen(currFwVersion)) +
    ...
    // Get the previous buffer size and cache it so the previous settings can be
    // restored.
    ...
    do {
      // Size adjuts dynamically to the current length of the currChunk number
      // to ensure we don't cut it out of the topic string.
      ...
      if (!result) {
        Logger::log("Failed chunk request...");
        retries--;
        if (!connected()) {
          Logger::log("Disconnected from server, reconnecting...");
          const bool connectResult = connect(TB_host, TB_token);
          if (connectResult) {
            Logger::log("Reconnection success");
          } else {
      ...
      // Amount of time we wait until we declare the download as failed in
      // milliseconds
      ...
      } while ((m_fwChunkReceive != currChunk) &&
               (millis() - startTime < timeout));

      // Check if downloaded the currentChunk timed out, before the packet was
      // received
      ...
      // Current chunck was downloaded successfully, resetting retries
      // accordingly.
    ...
    // to allow to receive ota chunck packets that might be much bigger than the
    // normal buffer size would allow, therefore we return to the previous value
    // to decrease overall memory usage
    if (changeBufferSize) {
    ...
    // additionaly clears and resets all variables connected to the update
    // itself
    Firmware_OTA_Unsubscribe();
  }

#endif  // THINGSBOARD_ENABLE_OTA
  /// @brief Connects to the previously set ThingsBoard server, as the given
  /// client with the given access token
  /// @param access_token Access token that connects this device with a created
  /// device on the ThingsBoard server, can be "provision", if the device
  /// creates itself instead
  /// @param client_id Client username that can be used to differentiate the
  ...
  inline bool connect_to_host(const char *access_token, const char *client_id,
    ...
    const bool connection_result =
        m_client.connect(client_id, access_token, password);
    ...
    this->Shared_Attributes_Unsubscribe();   // Cleanup all shared attributes
                                             // subscriptions
    this->Attributes_Request_Unsubscribe();  // Cleanup all client-side or
                                             // shared attributes requests
...
#if !THINGSBOARD_ENABLE_DYNAMIC
  /// @brief Reserves size for the given amount of items in our internal
  /// callback vectors beforehand for performance reasons, this ensures the
  /// internal memory blocks do not have to move if new data is inserted,
  /// because the current memory block is too small to hold the data for all the
  /// values
  /// @param reservedSize Size we want to reserve and should not use more if we
  /// do, the internal memory blocks might need to be moved to a new location
  ...
  /// @param callback Callback method that will be called
  /// @param registeredCallback Editable pointer to a reference of the local
  /// version that was copied from the passed callback
  /// @return Whether requesting the given callback was successful or not
  inline bool RPC_Request_Subscribe(
      const RPC_Request_Callback &callback,
      RPC_Request_Callback *&registeredCallback = nullptr) {
#if !THINGSBOARD_ENABLE_DYNAMIC
  ...
  /// version that was copied from the passed callback
  /// @return Whether requesting the given callback was successful or not
  inline bool Attributes_Request_Subscribe(
      const Attribute_Request_Callback &callback,
      Attribute_Request_Callback *&registeredCallback = nullptr) {
#if !THINGSBOARD_ENABLE_DYNAMIC
    if (m_attributeRequestCallbacks.size() + 1 >
        m_attributeRequestCallbacks.capacity()) {
  ...
  /// @brief Attempts to send a single key-value pair with the given key and
  /// value of the given type
  /// @tparam T Type of the passed value
  /// @param key Key of the key value pair we want to send
  /// @param value Value of the key value pair we want to send
  /// @param telemetry Whether the data we want to send should be sent as an
  /// attribute or telemetry data value
  /// @return Whether sending the data was successful or not
  template <typename T>
  inline bool sendKeyValue(const char *key, T value, bool telemetry = true) {
    ...
    return telemetry ? sendTelemetryJson(object,
                                         JSON_STRING_SIZE(measureJson(object)))
                     : sendAttributeJSON(object,
                                         JSON_STRING_SIZE(measureJson(object)));
  }

  /// @brief Process callback that will be called upon client-side RPC response
  /// arrival and is responsible for handling the payload and calling the
  /// appropriate previously subscribed callbacks
  /// @param topic Previously subscribed topic, we got the response over
  /// @param payload Payload that was sent over the cloud and received over the
  /// given topic
  /// @param length Total length of the received payload
  inline void process_rpc_request_message(char *topic, uint8_t *payload,
                                          const uint32_t length) {
#if THINGSBOARD_ENABLE_DYNAMIC
    // Buffer that we deserialize is writeable and not read only --> zero copy,
    // meaning the size for the data is 0 bytes, Data structure size depends on
    // the amount of key value pairs received. See
    // https://arduinojson.org/v6/assistant/ for more information on the needed
    // size for the JsonDocument
    const uint32_t dataStructureMemoryUsage = JSON_OBJECT_SIZE(
        getOccurences(reinterpret_cast<char *>(payload), COLON));
    TBJsonDocument jsonBuffer(dataStructureMemoryUsage);
#else
    StaticJsonDocument<JSON_OBJECT_SIZE(MaxFieldsAmt)> jsonBuffer;
#endif  // !THINGSBOARD_ENABLE_DYNAMIC

    // The deserializeJson method we use, can use the zero copy mode because a
    // writeable input was passed, if that were not the case the needed
    // allocated memory would drastically increase, because the keys would need
    // to be copied as well. See https://arduinojson.org/v6/doc/deserialization/
    // for more info on ArduinoJson deserialization
    const DeserializationError error =
        deserializeJson(jsonBuffer, payload, length);
    if (error) {
      char message[detectSize(UNABLE_TO_DE_SERIALIZE_JSON, error.c_str())];
      snprintf_P(message, sizeof(message), UNABLE_TO_DE_SERIALIZE_JSON,
                 error.c_str());
      Logger::log(message);
      return;
    }
    // .as() is used instead of .to(), because it is meant to cast the
    // JsonDocument to the given type, but it does not change the actual content
    // of the JsonDocument, we don't want that because it already contents
    // content and would result in the data simply being "null", instead .as()
    // allows accessing the data over a JsonVariantConst instead.
    ...
    for (size_t i = 0; i < m_rpcRequestCallbacks.size(); i++) {
      ...
      // Delete callback because the changes have been requested and the
    ...
    // if we are not waiting for any further responses with shared attributes
    // from the server. Will be resubscribed if another request is sent anyway
    if (m_rpcRequestCallbacks.empty()) {
      RPC_Request_Unsubscribe();
    }
  }

  /// @brief Process callback that will be called upon server-side RPC request
  /// arrival and is responsible for handling the payload and calling the
  /// appropriate previously subscribed callbacks
  /// @param topic Previously subscribed topic, we got the response over
  /// @param payload Payload that was sent over the cloud and received over the
  /// given topic
  /// @param length Total length of the received payload
  inline void process_rpc_message(char *topic, uint8_t *payload,
                                  const uint32_t length) {
    ...
    for (const RPC_Callback &callback : m_rpcCallbacks) {
      ...
      // Strncmp returns the ascis value difference of the ascii characters that
      // are different, meaning 0 is the same string and less and more than 0 is
      // the difference in ascci values between the 2 chararacters.
      else if (strncmp(subscribedMethodName, methodName,
                       strlen(subscribedMethodName)) != 0) {
        continue;
      }
      ...
      const JsonVariantConst param =
          data[RPC_PARAMS_KEY].as<JsonVariantConst>();
    ...
    snprintf_P(responseTopic, sizeof(responseTopic), RPC_SEND_RESPONSE_TOPIC,
               requestId);
  ...
  /// and is responsible for handling the payload and calling the appropriate
  /// previously subscribed callback
  /// @param topic Previously subscribed topic, we got the response over
  /// @param payload Payload that was sent over the cloud and received over the
  /// given topic
  /// @param length Total length of the received payload
  inline void process_firmware_response(char *topic, uint8_t *payload,
                                        const uint32_t length) {
    ...
    if (m_fwChunkReceive == 0) {
      ...
      if (!Update.begin(m_fwSize)) {
        ...
        // Ensure to call Update.abort after calling Update.begin even if it
        // failed, to make sure that any possibly started processes are stopped
        // and freed.
#ifdef ESP32
        Update.abort();
#endif
    ...
    char actual[JSON_STRING_SIZE(strlen(HASH_ACTUAL)) +
                JSON_STRING_SIZE(m_fwAlgorithm.size()) +
                JSON_STRING_SIZE(calculatedHash.size())];
    snprintf_P(actual, sizeof(actual), HASH_ACTUAL, m_fwAlgorithm.c_str(),
               calculatedHash.c_str());
    Logger::log(actual);

    char expected[JSON_STRING_SIZE(strlen(HASH_EXPECTED)) +
                  JSON_STRING_SIZE(m_fwAlgorithm.size()) +
                  JSON_STRING_SIZE(m_fwChecksum.size())];
    snprintf_P(expected, sizeof(expected), HASH_EXPECTED, m_fwAlgorithm.c_str(),
               m_fwChecksum.c_str());
    Logger::log(expected);

    // Check if the initally received checksum is the same as the one we
    // calculated from the received binary data, if not we assume the binary
    // data has been changed or not completly downloaded --> Firmware update
    // failed
  ...
  /// @brief Process callback that will be called upon shared attribute update
  /// arrival and is responsible for handling the payload and calling the
  /// appropriate previously subscribed callbacks
  /// @param topic Previously subscribed topic, we got the response over
  /// @param payload Payload that was sent over the cloud and received over the
  /// given topic
  /// @param length Total length of the received payload
  inline void process_shared_attribute_update_message(char *topic,
                                                      uint8_t *payload,
                                                      const uint32_t length) {
#if THINGSBOARD_ENABLE_DYNAMIC
    // Buffer that we deserialize is writeable and not read only --> zero copy,
    ...
    for (const Shared_Attribute_Callback &callback :
         m_sharedAttributeUpdateCallbacks) {
#if THINGSBOARD_ENABLE_STL
      ...
      do {
        ...
        // If no more commas are found, set the currentIndex to the end of the
        // string
        if (currentIndex == -1) {
        ...
        const char *att =
            stringToSplit.substring(previousIndex, currentIndex).c_str();
#endif  // THINGSBOARD_ENABLE_STL
        if (att == nullptr) {
          Logger::log(ATT_IS_NULL);
          continue;
        ...
        if (containsKey) {
          char contained_message[JSON_STRING_SIZE(strlen(ATT_IN_ARRAY)) +
                                 JSON_STRING_SIZE(strlen(att))];
          snprintf_P(contained_message, sizeof(contained_message), ATT_IN_ARRAY,
                     att);
      ...
      snprintf_P(calling_message, sizeof(calling_message), CALLING_ATT_CB,
                 requested_att);
  ...
  /// @brief Process callback that will be called upon client-side or shared
  /// attribute request arrival and is responsible for handling the payload and
  /// calling the appropriate previously subscribed callbacks
  /// @param topic Previously subscribed topic, we got the response over
  /// @param payload Payload that was sent over the cloud and received over the
  /// given topic
  /// @param length Total length of the received payload
  inline void process_attribute_request_message(char *topic, uint8_t *payload,
                                                const uint32_t length) {
#if THINGSBOARD_ENABLE_DYNAMIC
    // Buffer that we deserialize is writeable and not read only --> zero copy,
    // meaning the size for the data is 0 bytes, Data structure size depends on
    ...
    for (size_t i = 0; i < m_attributeRequestCallbacks.size(); i++) {
      const Attribute_Request_Callback &callback =
          m_attributeRequestCallbacks.at(i);
    ...
    delete_callback:
      ...
      // Delete callback because the changes have been requested and the
      // callback is no longer needed
      m_attributeRequestCallbacks.erase(
          std::next(m_attributeRequestCallbacks.begin(), i));
#else
      // Delete callback because the changes have been requested and the
      // callback is no longer needed
  ...
  /// @brief Process callback that will be called upon provision response
  /// arrival and is responsible for handling the payload and calling the
  /// appropriate previously subscribed callback
  /// @param topic Previously subscribed topic, we got the response over
  /// @param payload Payload that was sent over the cloud and received over the
  /// given topic
  /// @param length Total length of the received payload
  inline void process_provisioning_response(char *topic, uint8_t *payload,
                                            const uint32_t length) {
  ...
  /// attribute or telemetry data value
  /// @return Whether sending the data was successful or not
  inline bool sendDataArray(const Telemetry *data, size_t data_count,
                            bool telemetry = true) {
#if THINGSBOARD_ENABLE_DYNAMIC
    // String are const char* and therefore stored as a pointer --> zero copy,
    // meaning the size for the strings is 0 bytes, Data structure size depends
    // on the amount of key value pairs passed. See
    // https://arduinojson.org/v6/assistant/ for more information on the needed
    // size for the JsonDocument
...
#if THINGSBOARD_ENABLE_STL
  // Vectors hold copy of the actual passed data, this is to ensure they stay
  // valid, even if the user only temporarily created the object before the
  // method was called. This can be done because all Callback methods mostly
  // consists of pointers to actual object so copying them does not require a
  // huge memory overhead and is acceptable especially in comparsion to possible
  // problems that could arise if references were used and the end user does not
  // take care to ensure the Callbacks live on for the entirety of its usage,
  // which will lead to dangling references and undefined behaviour
  std::vector<RPC_Callback> m_rpcCallbacks;  // Server side RPC callbacks vector
  std::vector<RPC_Request_Callback>
      m_rpcRequestCallbacks;  // Client side RPC callbacks vector
  std::vector<Shared_Attribute_Callback>
      m_sharedAttributeUpdateCallbacks;  // Shared attribute update callbacks
                                         // vector
  std::vector<Attribute_Request_Callback>
      m_attributeRequestCallbacks;  // Client-side or shared attribute request
                                    // callbacks vector
#else
  Vector<RPC_Callback> m_rpcCallbacks;  // Server side RPC callbacks vector,
                                        // replacement for non C++ STL boards
  Vector<RPC_Request_Callback>
      m_rpcRequestCallbacks;  // Client side RPC callbacks vector, replacement
                              // for non C++ STL boards
  Vector<Shared_Attribute_Callback>
      m_sharedAttributeUpdateCallbacks;  // Shared attribute update callbacks
                                         // vector, replacement for non C++ STL
                                         // boards
  Vector<Attribute_Request_Callback>
      m_attributeRequestCallbacks;  // Client-side or shared attribute request
                                    // callback vector, replacement for non C++
                                    // STL boards
#endif

  Provision_Callback m_provisionCallback;  // Provision response callback
  uint32_t m_requestId;  // Allows nearly 4.3 million requests before wrapping
                         // back to 0.
  bool m_qos;  // Whether QoS level 1 should be enabled or disabled (Resends the
               // packet until the message was received and a PUBACK packet was
               // returned).
  ...
  /// @brief MQTT callback that will be called if a publish message is received
  /// from the server
  /// @param topic Previously subscribed topic, we got the response over
  /// @param payload Payload that was sent over the cloud and received over the
  /// given topic
  ...
  inline void onMQTTMessage(char *topic, uint8_t *payload, uint32_t length) {
    char message[JSON_STRING_SIZE(strlen(CB_ON_MESSAGE)) +
                 JSON_STRING_SIZE(strlen(topic))];
    ...
    } else if (strncmp_P(RPC_REQUEST_TOPIC, topic, strlen(RPC_REQUEST_TOPIC)) ==
               0) {
      process_rpc_message(topic, payload, length);
    } else if (strncmp_P(ATTRIBUTE_RESPONSE_TOPIC, topic,
                         strlen(ATTRIBUTE_RESPONSE_TOPIC)) == 0) {
      process_attribute_request_message(topic, payload, length);
    } else if (strncmp_P(ATTRIBUTE_TOPIC, topic, strlen(ATTRIBUTE_TOPIC)) ==
               0) {
      process_shared_attribute_update_message(topic, payload, length);
    } else if (strncmp_P(PROV_RESPONSE_TOPIC, topic,
                         strlen(PROV_RESPONSE_TOPIC)) == 0) {
      process_provisioning_response(topic, payload, length);
    } else if (strncmp_P(FIRMWARE_RESPONSE_TOPIC, topic,
                         strlen(FIRMWARE_RESPONSE_TOPIC)) == 0) {
#if THINGSBOARD_ENABLE_OTA
  ...
  static void onStaticMQTTMessage(char *topic, uint8_t *payload,
                                  unsigned int length) {
...
ThingsBoardSized<Logger> *ThingsBoardSized<Logger>::m_subscribedInstance =
    nullptr;
