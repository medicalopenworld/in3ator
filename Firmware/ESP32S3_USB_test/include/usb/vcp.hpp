...
{
    // Replace static_assert with a runtime check
    if (T::pids.begin() == nullptr) {
        throw std::runtime_error("Every VCP driver must contain an array of supported PIDs in 'pids' array");
    }
    if (T::vid == 0) {
        throw std::runtime_error("Every VCP driver must contain supported VID in 'vid' integer");
    }
...
typedef struct vcp_driver {
    ...
    // Replace 'auto' with the specific function pointer type
    vcp_driver(CdcAcmDevice *(*open_func)(uint16_t pid, const cdc_acm_host_device_config_t *dev_config, uint8_t interface_idx), 
               const uint16_t _vid, 
               const std::vector<uint16_t> &_pids)
        : open(open_func), vid(_vid), pids(_pids) {};
} vcp_driver;
